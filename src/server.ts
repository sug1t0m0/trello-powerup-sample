...
function validateEnvSetup() {
  ...
  for (const env of requiredEnv) {
    if (!actualEnv.includes(env)) {
      console.error(
        "You are missing Environmental Variables! Make sure you create a .env file or set these natively. Exiting."
      );
      for (const missingEnv of requiredEnv.filter(
        (e) => !actualEnv.includes(e)
      )) {
